...
public class CommitMessageService {
    ...
    public String generateCommitMessage(String diff, Project currentProject) throws Exception{
        String prompt = PromptUtil.constructPrompt(diff, currentProject);
        return aiService.generateCommitMessage(prompt);
    }

    public void generateCommitMessageStream(String diff, Project currentProject, Consumer<String> onNext, Consumer<Throwable> onError) throws Exception{
        String prompt = PromptUtil.constructPrompt(diff, currentProject);
